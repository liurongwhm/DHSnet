...
from utils_HSI import open_file2, open_file1
...
def get_dataset(dataset_name, target_folder="./", datasets=DATASETS_CONFIG):
    ...
    if dataset_name == 'Houston13':
        # Load the image
        img = open_file2(folder + 'Houston13.mat')['ori_data']
        img = torch.from_numpy(img).permute(1, 2, 0)

        rgb_bands = (43, 21, 11)

        gt = open_file2(folder + 'Houston13_7gt.mat')['map']
        ...
        # Load the image
        img = open_file2(folder + 'Houston18.mat')['ori_data']
        img = torch.from_numpy(img).permute(1, 2, 0)

        rgb_bands = (43, 21, 11)

        gt = open_file2(folder + 'Houston18_7gt.mat')['map']
        ...
        # Load the image
        img = open_file1(folder + 'Dioni.mat')['ori_data']
        img = torch.from_numpy(img.astype(np.float32))

        rgb_bands = (43, 21, 11)

        gt = open_file1(folder + 'Dioni_gt_out68.mat')['map']
        ...
        # Load the image
        img = open_file1(folder + 'Loukia.mat')['ori_data']
        img = torch.from_numpy(img.astype(np.float32))
        rgb_bands = (43, 21, 11)

        gt = open_file1(folder + 'Loukia_gt_out68.mat')['map']
        ...
        # Load the image
        img = open_file1(folder + 'paviaU.mat')['ori_data']
        # img = torch.from_numpy(img).permute(1, 2, 0)

        rgb_bands = (43, 21, 11)

        gt = open_file1(folder + 'paviaU_7gt.mat')['map']
        ...
        # Load the image
        img = open_file1(folder + 'paviaC.mat')['ori_data']
        # img = torch.from_numpy(img).permute(1, 2, 0)

        rgb_bands = (43, 21, 11)

        gt = open_file1(folder + 'paviaC_7gt.mat')['map']
    ...
    # Filter NaN out
    if dataset_name in ['PaviaU', 'PaviaC']:
        nan_mask = np.isnan(img.sum(axis=-1))  # Pavia
        preprocess = False
    else:
        nan_mask = np.isnan(img.sum(axis=-1)).bool()
        preprocess = True
